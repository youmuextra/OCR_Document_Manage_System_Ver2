...
class LLMService:
    ...
    @staticmethod
    def _get_ollama_tags(timeout_sec: int = 3):
        """读取本地 Ollama 已安装模型列表。"""
        try:
            resp = requests.get("http://localhost:11434/api/tags", timeout=timeout_sec)
    ...
    @staticmethod
    def check_ollama_runtime(timeout_sec: int = 3) -> Dict[str, Any]:
        """检测 Ollama 是否已安装、服务是否可用、目标模型是否已就绪。"""
        installed = False
        ...
        if installed:
            tags = LLMService._get_ollama_tags(timeout_sec=timeout_sec)
            server_running = tags is not None
            if isinstance(tags, dict):
                models = tags.get("models") or []
                names = [str(m.get("name") or "").strip() for m in models if isinstance(m, dict)]
                model_ready = LLMService.MODEL_NAME in names
    ...
    @staticmethod
    def start_ollama_service(wait_seconds: int = 6) -> Dict[str, Any]:
        """尝试启动 Ollama 服务（若未启动）。"""
        status = LLMService.check_ollama_runtime(timeout_sec=2)
        if status.get("server_running"):
            return {"ok": True, "message": "Ollama 服务已在运行"}
        ...
        try:
            ...
            return {"ok": False, "message": f"启动 Ollama 失败: {e}"}

        for _ in range(max(wait_seconds, 1) * 2):
            time.sleep(0.5)
            tags = LLMService._get_ollama_tags(timeout_sec=2)
            if tags is not None:
                return {"ok": True, "message": "Ollama 服务启动成功"}

        return {"ok": False, "message": "Ollama 服务未在预期时间内就绪"}

    @staticmethod
    def pull_model(timeout_sec: int = 1800) -> Dict[str, Any]:
        """拉取当前配置模型。"""
        try:
            r = subprocess.run(
                ["ollama", "pull", LLMService.MODEL_NAME],
                capture_output=True,
                text=True,
                timeout=timeout_sec,
            )
            ...
            return {"ok": False, "message": f"模型下载失败: {msg}"}
        except Exception as e:
            return {"ok": False, "message": f"模型下载异常: {e}"}
